...
async function main() {
  console.log('Seeding database with 5,000+ Deals & Outlets...');

  // Seed Default AdminSettings with Qwerty123!@#
  ...
  await prisma.merchant.upsert({
    ...
    create: {
      ...
      branchesJson: JSON.stringify([
        ...
        { name: 'Highway Outlet', address: 'Main Super Highway, Near Toll Plaza, Karachi', phone: '021 34381111' }
      ])
    }
  });

  // Comprehensive 5,000+ Deals Database across Restaurants, Medical Labs, Fashion, Electronics, etc.
  const merchantsList = [
    ...
    for (const m of merchantsList) {
      ...
      await prisma.deal.create({
        data: {
          ...
          description: `${discountVal}% off total bill / test package at ${m.name} using ${bankObj?.fullName || 'Bank Card'}`,
          ...
          validTo: '2026-12-31',
          termsJson: JSON.stringify(['Present valid card at billing', 'Verified by Peekaboo Guru Partner Network']),
          featured: count % 4 === 0,
          trending: count % 3 === 0,
  ...
  console.log(`Database seeded with ${count} active deals across all categories & outlets nationwide!`);
}
