/* ============================================
   CardSaver.pk — Data Layer (Phase 7 Peekaboo Full Match)
   10 Categories (with Places & Deals counts), 16 Bank Partners Grid,
   5,000+ Deals Generation Engine
   ============================================ */

const Data = {
  // ── 30 Banks & Digital Wallets ──
  banks: [
  ...
  // ── 10 Peekaboo Categories with Places & Deals counts (Screenshot Match) ──
  categories: [
  ...
  // ── Card Tiers ──
  cardTiers: [
    { id: 'Classic', name: 'Classic / Green', badgeColor: '#8E8E93', priority: 1 },
    { id: 'Silver', name: 'Silver', badgeColor: '#A0A0A0', priority: 2 },
    { id: 'Gold', name: 'Gold', badgeColor: '#FFD700', priority: 3 },
    { id: 'Titanium', name: 'Titanium', badgeColor: '#707070', priority: 4 },
    { id: 'Platinum', name: 'Platinum', badgeColor: '#E5E4E2', priority: 5 },
    { id: 'Signature', name: 'Signature (Visa)', badgeColor: '#4B0082', priority: 6 },
    { id: 'World', name: 'World (Mastercard)', badgeColor: '#2C2C2C', priority: 7 },
    { id: 'Infinite', name: 'Infinite (Visa)', badgeColor: '#111111', priority: 8 },
    { id: 'PayPak Green', name: 'PayPak Classic', badgeColor: '#00833D', priority: 1 },
    { id: 'PayPak Gold', name: 'PayPak Gold', badgeColor: '#B8860B', priority: 2 },
  ],

  cities: ['Karachi', 'Lahore', 'Islamabad', 'Rawalpindi', 'Faisalabad', 'Peshawar', 'Multan', 'Quetta', 'Sialkot', 'Hyderabad', 'Gujranwala', 'Sukkur', 'Abbottabad'],
  ...
  getTier(tierId) {
    return this.cardTiers.find(t => t.id === tierId);
  },

  // ── Enriched Merchant Database ──
  merchantsData: {
    'Kababjees': {
      name: 'Kababjees',
  ...
  getMerchant(name) {
    if (this.merchantsData[name]) return this.merchantsData[name];
    return {
      name: name,
      logo: 'assets/images/category_restaurants.png',
      banner: 'assets/images/category_restaurants.png',
      description: `${name} is one of Pakistan's favorite dining and shopping partners offering exclusive cardholder discounts.`,
      rating: 4.2,
      reviewCount: 45,
      branchesCount: 3,
      dealsCount: 12,
      ...
      social: {
        ...
        website: `https://${name.toLowerCase().replace(/\s+/g, '')}.pk`
      },
      menu: [
        { name: `${name} Special Combo`, price: 'PKR 1,499', category: 'Deals', image: '🍱' },
      ...
      branches: [
        { name: 'Main City Outlet', address: 'Commercial Market, Central City', phone: '111 000 786' }
      ]
    };
  },

  getFollowedMerchants() {
    return Utils.storage.get('followed_merchants') || [];
  },

  isMerchantFollowed(name) {
    const list = this.getFollowedMerchants();
    return list.includes(name);
  },

  toggleFollowMerchant(name) {
    let list = this.getFollowedMerchants();
    if (list.includes(name)) {
      list = list.filter(m => m !== name);
      Utils.showToast('Unfollowed', `You unfollowed ${name}`, 'info');
    } else {
      list.push(name);
      Utils.showToast('Following! ❤️', `You are now following ${name}`, 'success');
    }
    Utils.storage.set('followed_merchants', list);
    return list.includes(name);
  },

  // ── Get Deals (Scraped Live or Scaled 500+ Deals) ──
  getDeals() {
    const scraped = Utils.storage.get('live_peekaboo_deals');
    if (scraped && Array.isArray(scraped) && scraped.length > 0) {
      return scraped;
    }
    return this.generate500PlusDeals();
  },

  saveLiveDeals(deals) {
    Utils.storage.set('live_peekaboo_deals', deals);
    Deals.allDeals = deals;
  },

  clearLiveDeals() {
    Utils.storage.remove('live_peekaboo_deals');
    Deals.allDeals = null;
  },

  // ── 500+ Deals Generator across all cities & categories ──
  generate500PlusDeals() {
    const merchantsList = [
    ...
    for (let i = 0; i < 50; i++) {
      merchantsList.forEach(m => {
        ...
        deals.push({
          ...
          description: `${discountVal}% off total bill at ${m.name} using ${Data.getBank(bankIds[bankIndex])?.fullName || 'Bank Card'}`,
          banks: [bankIds[bankIndex], bankIds[(bankIndex + 1) % bankIds.length]],
          cardTypes: ['Credit Card', 'Debit Card'],
          tiers: ['Classic', 'Gold', 'Platinum', 'Signature'],
          cities: [citiesList[cityIndex]],
          logo: m.logo,
          banner: m.banner,
          validFrom: '2026-08-01',
          validTo: '2026-12-31',
          terms: ['Present valid debit/credit card at billing', 'Source: Peekaboo Guru Partner Network'],
          featured: count % 3 === 0,
          trending: count % 2 === 0,
  ...
  getMatchingDeals(userCards, allDeals) {
    if (!userCards || userCards.length === 0) return [];
    const userBankIds = [...new Set(userCards.map(c => c.bank))];
    const userCardTypes = [...new Set(userCards.map(c => c.type))];
    const userTiers = [...new Set(userCards.map(c => c.tier || 'Classic'))];

    return allDeals.filter(deal => {
      const bankMatch = deal.banks.some(b => userBankIds.includes(b));
      const typeMatch = deal.cardTypes.some(t => userCardTypes.includes(t));
      const tierMatch = !deal.tiers || deal.tiers.some(t => userTiers.includes(t));
      return bankMatch && typeMatch && tierMatch;
    });
  },

  getGroupDeals(groupCards, allDeals) {
    return this.getMatchingDeals(groupCards, allDeals);
  },
